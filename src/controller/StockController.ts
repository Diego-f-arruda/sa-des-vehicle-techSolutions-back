...
export async function stockController(app: FastifyInstance){
    ...
    app.post("/stock", async (request: FastifyRequest, reply) => {
        const body = request.body as { acessorioId: string, nome: string, tipoAcessorio: TipoAcessorio, quantidade: number};
        
        

        try {
            await stockService.create(body.acessorioId, body.nome, body.tipoAcessorio, body.quantidade);
    ...
    app.patch("/stock/:id/quantidade",  async(request, reply) => {
        ...
        try {
            const stock = await stockService.updateQuantidade(id);
